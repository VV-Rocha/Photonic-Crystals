...
class StorageField(StorageFieldMethods, FieldDirectories):
    """ Class to handle storage of simulation fields."""
    pass

class CoupledStorageField(CoupledFieldDirectories):
    """ Class to handle storage of coupled simulation fields."""
    def store_step(self, index: str | int | None=None):
        """ Store the coupled fields at a given step based on storage mode.

        Args:
            index (str | int | None, optional): step index to store. Defaults to None.
        """
        if self.store.lower() == "last":
            if index == self.Nsteps:
                self.store_field(index="last")
    ...
    def store_field(self, index: str | int | None=None):
